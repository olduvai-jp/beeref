...
def create_test_image_file(filepath, width=10, height=10, color=QtCore.Qt.GlobalColor.red):
    """テスト用の画像ファイルを作成"""
...
class TestCreateSequenceItemFromFiles:
    ...
    def test_create_mixed_valid_invalid_files(self, qapp, test_sequence_files, temp_dir):
        """有効・無効ファイル混在での作成"""
...
class TestImportSequencesFromDirectory:
    ...
    def test_import_with_sequences_found(self, mock_detect, qapp, test_sequence_files):
        ...
        with patch('beeref.fileio.sequence_import.create_sequence_item_from_group') as mock_create:
            mock_item = MagicMock(spec=BeeSequenceItem)
            mock_item.filename = 'Test Sequence'
            mock_create.return_value = mock_item
            
            items = import_sequences_from_directory('/fake/path', fps=24.0, min_sequence_length=3)
...
class TestIntegration:
    ...
    def test_full_workflow_integration(self, qapp, temp_dir):
        ...
        # 3. シーケンス作成
        item = create_sequence_item_from_files(sequence_files, fps=info['suggested_fps'])
        assert item is not None
