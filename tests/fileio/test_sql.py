...
from beeref.items import BeePixmapItem, BeeTextItem, BeeErrorItem, BeeAnimatedPixmapItem
...
class TestAnimatedPixmapRestoration:
    """アニメーション画像復元のテスト"""
    
    def test_restore_animated_pixmap_success(self, qapp):
        """正常なアニメーション画像復元のテスト"""
        # テスト用のアニメーションデータを準備
        frames = []
        for i in range(2):
        ...
        # 元のアイテムを作成してバイト列に変換
        original_item = BeeAnimatedPixmapItem(animation_data, filename='test.gif')
        original_item.current_frame = 1
        data_bytes, format_type = original_item.pixmap_to_bytes()
        
        # モックシーンを作成
        mock_scene = MagicMock()
        mock_scene.add_item_later = MagicMock()
        
        # SQLiteIOを使ってアイテムを復元
        io = SQLiteIO(':memory:', mock_scene, create_new=True)
        io._establish_connection()
        io.create_schema_on_new()
        
        # データベースにアイテムデータを挿入（現在のスキーマに合わせて）
        cursor = io.ex("""INSERT INTO items (
            type, x, y, z, scale, rotation, flip, data
        ) VALUES ('animated_pixmap', 10, 20, 0.5, 1.0, 45, 1, ?)""",
                      (json.dumps({'filename': 'test.gif'}),))
        item_id = cursor.lastrowid
        
        # sqlarテーブルに画像データを挿入
        io.ex("""INSERT INTO sqlar (name, item_id, mode, sz, data)
                 VALUES ('animated_data', ?, 33188, ?, ?)""",
              (item_id, len(data_bytes), data_bytes))
        io.connection.commit()
        
        # 復元処理を実行
        with patch('beeref.fileio.sql.logger'):
            io.read()
        
        # add_item_laterが適切なデータで呼ばれたことを確認
        mock_scene.add_item_later.assert_called_once()
        call_args = mock_scene.add_item_later.call_args[0][0]
    ...
    def test_restore_animated_pixmap_invalid_data(self, qapp):
        ...
        # 無効なバイトデータを挿入
        invalid_data = b'invalid animation data'
        cursor = io.ex("""INSERT INTO items (
            type, x, y, z, scale, rotation, flip, data
        ) VALUES ('animated_pixmap', 10, 20, 0.5, 1.0, 45, 1, ?)""",
                      (json.dumps({'filename': 'test.gif'}),))
        ...
        io.ex("""INSERT INTO sqlar (name, item_id, mode, sz, data)
                 VALUES ('animated_data', ?, 33188, ?, ?)""",
              (item_id, len(invalid_data), invalid_data))
        io.connection.commit()
        ...
        assert isinstance(call_args['item'], BeeErrorItem)
        assert 'Animated image could not be loaded' in call_args['data']['text']
    
    def test_restore_animated_pixmap_null_frames(self, qapp):
        """復元されたフレームがnullの場合のエラーテスト"""
        mock_scene = MagicMock()
        mock_scene.add_item_later = MagicMock()
        ...
        invalid_animation_data = {
            'frames': ['invalid_hex_data'],
            'delays': [100],
            'current_frame': 0
        }
        json_str = json.dumps(invalid_animation_data)
    ...
    def test_restore_animated_pixmap_exception_handling(self, qapp):
        """復元中の例外発生時のハンドリングテスト"""
        mock_scene = MagicMock()
        mock_scene.add_item_later = MagicMock()
        ...
        io.ex("""INSERT INTO sqlar (name, item_id, mode, sz, data)
                 VALUES ('animated_data', ?, 33188, ?, ?)""",
              (item_id, 9, b'some_data'))
        io.connection.commit()
        
        # BeeAnimatedPixmapItemの初期化で例外を発生させる
        with patch('beeref.fileio.sql.BeeAnimatedPixmapItem',
                   side_effect=Exception("Initialization failed")), \
             patch('beeref.fileio.sql.logger'):
