...
class TestInitialization:
    ...
    def test_init_basic(self, selectable_mock, qapp, gif_data):
        ...
        assert item._frame_cache == {0: item._frame_cache[0]}  # 最初のフレームがキャッシュされている
...
class TestFrameAccess:
    ...
    def test_frame_cache_size_limit(self, mock_logger, qapp, animated_data_item):
        """フレームキャッシュサイズ制限のテスト"""
...
class TestCompatibilityInterface:
    ...
    def test_pixmap_returns_current_frame(self, qapp, animated_data_item):
        ...
        expected_pixmap = animated_data_item.get_frame_pixmap(animated_data_item._current_frame)
...
class TestAnimationControl:
    ...
    def test_start_animation_multiple_frames_with_scene(self, qapp, animated_data_item):
...
class TestSceneIntegration:
    ...
    def test_item_change_added_to_scene(self, mock_start, qapp, animated_data_item):
    ...
    def test_item_change_removed_from_scene(self, mock_stop, qapp, animated_data_item):
...
class TestSerialization:
    ...
    def test_create_from_data_minimal(self, qapp, animated_data_item):
        ...
        result = BeeAnimatedDataItem.create_from_data(item=animated_data_item, data=data)
    ...
    def test_pixmap_to_bytes_preserves_original_data(self, qapp, animated_data_item):
...
class TestMemoryEfficiency:
    ...
    def test_frame_cache_memory_management(self, qapp, animated_data_item):
        """フレームキャッシュのメモリ管理"""
        # 初期状態でキャッシュサイズを確認
        initial_cache_size = len(animated_data_item._frame_cache)
...
class TestSameAsSourceExport:
    ...
    def test_to_same_as_source_bytes_no_crop(self, qapp, animated_data_item):
        ...
        data, format_type = animated_data_item.to_same_as_source_bytes(apply_crop=False)
    ...
    def test_to_same_as_source_bytes_with_crop(self, qapp, animated_data_item):
        ...
        data, format_type = animated_data_item.to_same_as_source_bytes(apply_crop=True)
    ...
    @patch('beeref.items.logger')
    def test_to_same_as_source_bytes_error_handling(self, mock_logger, qapp, animated_data_item):
        """Same as Source エクスポートのエラーハンドリング"""
        # 最終フォールバック処理をモック
        with patch.object(animated_data_item, 'to_animated_gif_bytes') as mock_gif_fallback:
            mock_gif_fallback.return_value = (b'fallback_data', 'gif')
            
            # 故意にエラーを発生させる（get_imgformatが常にエラーを投げる）
            with patch.object(animated_data_item, 'get_imgformat', side_effect=Exception("Test error")):
                data, format_type = animated_data_item.to_same_as_source_bytes(apply_crop=False)
...
class TestExportFormatCompatibility:
    ...
    def test_export_format_consistency(self, qapp, animated_data_item):
        """各エクスポート形式の一貫性確認"""
        # Same as Source
        same_data, same_format = animated_data_item.to_same_as_source_bytes(apply_crop=False)
        
        # GIF形式
        gif_data, gif_format = animated_data_item.to_animated_gif_bytes(apply_crop=False)
        
        # WebP形式
        webp_data, webp_format = animated_data_item.to_animated_webp_bytes(apply_crop=False)
...
class TestResourceManagement:
    ...
    def test_destructor_cleanup(self, qapp, gif_data):
        ...
        assert item._buffer.isOpen() if hasattr(item._buffer, 'isOpen') else True
