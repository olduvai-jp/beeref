...
class BeeGraphicsView(MainControlsMixin,
    ...
    def on_action_export_selected_images(self):
        # Check if any image items are selected
        ...
        directory = QtWidgets.QFileDialog.getExistingDirectory(
            parent=self,
            caption='Export Selected Images',
            directory=directory)
    ...
    def on_selection_changed(self):
        # Check if scene is still valid (not deleted by Qt)
        try:
            if not self.scene or not hasattr(self.scene, 'selectedItems'):
                return
