...
class FolderImporter:
    ...
    def import_folder(self, folder_path: str, pos: Tuple[float, float] = (0, 0)) -> FolderImportResult:
        """
        フォルダから画像をインポート
    ...
    def _process_sequence(self, sequence, pos: Tuple[float, float]) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.debug(f"Processing sequence: {sequence_info['prefix']}*{sequence_info['suffix']}")
            ...
            if not animation_data:
                logger.warning(f"Failed to create animation from sequence")
                return None
            ...
            item_data = {
                'type': 'animated_data',
                'data': animation_data,
                'filename': f"sequence_{sequence_info['prefix']}_{len(image_paths)}_frames",
                'pos': pos,
                'sequence_info': sequence_info
            }
            
            logger.debug(f"Created animation item data for sequence")
            return item_data
    ...
    def _process_static_image(self, file_path: str, pos: Tuple[float, float]) -> Optional[Dict[str, Any]]:
        """
...
def import_folder_images(folder_path: str, 
...
def create_items_from_import_result(result: FolderImportResult, 
